...
function get_soundcloud_thumbnail($type, $id, $image_size = 't300xt300') {
	if (!function_exists('curl_init')) {
		return null;
	} else {
		global $options;
	...
	//if (is_single() || is_page()) {
		...
		// YOUTUBE
			...
			foreach($matches[1] as $match) {
				...
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- YouTube object: $yt_thumbnail (ID:$match) -->\n\r";
				}
			...
			foreach($matches[1] as $match) {
				$yt_thumbnail = 'http://img.youtube.com/vi/' . $match . '/0.jpg';
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- YouTube iFrame: $yt_thumbnail (ID:$match) -->\n\r";
				}
				if (isset($yt_thumbnail)) {
				  $og_thumbnails[] = $yt_thumbnail;
				}
		...
		// VIMEO
			...
			foreach($matches[1] as $match) {
				$vm_thumbnail = get_vimeo_thumbnail($match, VIMEO_IMAGE_SIZE);
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- Vimeo object: $vm_thumbnail (ID:$match) -->\n\r";
				}
			...
			foreach($matches[1] as $match) {
				$vm_thumbnail = get_vimeo_thumbnail($match, VIMEO_IMAGE_SIZE);
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- Vimeo iFrame: $vm_thumbnail (ID:$match) -->\n\r";
				}
				if (isset($vm_thumbnail)) {
				  $og_thumbnails[] = $vm_thumbnail;
				}
		...
		// DAILYMOTION
			...
			foreach($matches[1] as $match) {
				...
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- Dailymotion Flash: $dailymotion_thumbnail (ID:$match) -->\n\r";
				}
			...
			foreach($matches[1] as $match) {
				$dailymotion_thumbnail = get_dailymotion_thumbnail($match);
				if(OGRAPHR_DEBUG == TRUE) {
					print "<!-- Dailymotion iFrame: $dailymotion_thumbnail  (ID:$match) -->\n\r";
				}
				if (isset($dailymotion_thumbnail)) {
					$dailymotion_thumbnail = preg_replace('/\?([A-Za-z0-9]+)/', '', $dailymotion_thumbnail); // remove suffix
					$og_thumbnails[] = $dailymotion_thumbnail;
		...
		// SOUNDCLOUD
			...
			foreach($matches[1] as $match) {
				$sc_thumbnail = get_soundcloud_thumbnail('tracks', $match, SOUNDCLOUD_IMAGE_SIZE);
				if(OGRAPHR_DEBUG == TRUE) {
			...
			// Now if we've found a SoundCloud ID, let's set the thumbnail URL
			foreach($matches[1] as $match) {
				$sc_thumbnail = get_soundcloud_thumbnail('playlists', $match, SOUNDCLOUD_IMAGE_SIZE);
				if(OGRAPHR_DEBUG == TRUE) {
		...
		// MIXCLOUD	
			...
			foreach($matches[1] as $match) {
				...
				$mixcloud_thumbnail = get_mixcloud_thumbnail($mixcloud_id, MIXCLOUD_IMAGE_SIZE);
				if(OGRAPHR_DEBUG == TRUE) {
